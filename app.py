...
#     st.success(f"Welcome {user_name}! Start your webcam below.")
...
#                 result = self.model.predict(face, verbose=0)
#                 emotion_index = np.argmax(result)
#                 emotion = emotion_names[emotion_index]

#                 cv2.rectangle(img, (x, y), (x+w, y+h), (0, 255, 0), 2)
#                 cv2.putText(
#                     img, f"{self.user_name}: {emotion}", (x, y-10),
...
#     )
# else:
#     st.warning("Please enter your name to start the webcam")
    ...
    class EmotionDetector(VideoProcessorBase):
        ...
        def recv(self, frame):
            ...
            for (x, y, w, h) in faces:
                ...
                cv2.rectangle(img, (x, y), (x+w, y+h), (0, 255, 0), 2)
                cv2.putText(
                    img, f"{user_name}: {emotion}", (x, y-10),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.9, (255, 0, 0), 2
                )
